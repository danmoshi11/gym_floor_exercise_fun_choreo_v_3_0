...
const HeroEngine = {
    ...
    galleryImages: [
        ...
        './images/gallery/Alice_DAmato_Showcast.png', './images/gallery/Kaylia_Nemour_Showcast.jpg',
        ...
        oldNodes.forEach(node => {
            if (isSwitchingMode) {
                ...
                if (node.classList.contains('gallery-blurred')) {
                    ...
                    node.style.opacity = '0';
                    node.style.transition = 'opacity 1.2s ease-out, filter 1.2s ease-out';
                }
            }
        });
    ...
    playVideoMode: function(isSwitchingToNext) {
        ...
        let isReady = false; // 缓冲状态锁
        const durationMs = videoObj.duration * 1000;

        // ✨ 核心机制：只有当视频缓冲了足够的数据、可以流畅播放时，才触发时间轴！
        videoEl.oncanplay = () => {
            if (isReady) return;
            isReady = true;

            // 🎯 接管首屏 Loading 销毁权：如果是用户刚打开网页，在这里彻底驱散 Loading 界面！
            const loader = document.getElementById('globalLoader');
            if (loader && !loader.classList.contains('opacity-0')) {
                const progress = document.getElementById('loaderProgress');
                if(progress) progress.style.width = '100%'; // 进度条拉满
                setTimeout(() => {
            ...
            // --- 视频准备就绪，真正的时间轴才开始倒数！ ---
            this.addSceneTimer(() => { videoEl.style.opacity = '1'; }, 0);
        ...
        // 🚨 终极防卡死阀门：万一网络极其卡顿，缓冲超过了 10 秒钟还没好，直接丢弃该视频，防止用户永久黑屏！
        this.addSceneTimer(() => {
            if (!isReady) {
                console.error(`❌ 视频缓冲超时: ${videoObj.src}，已强制切到下一幕`);
                isReady = true; // 锁死，防止后续突然加载出来
                videoEl.oncanplay = null;
                
                // 万一是首屏卡死了，也要把 Loading 撤掉，别让用户卡死在加载页
                const loader = document.getElementById('globalLoader');
                if (loader && !loader.classList.contains('opacity-0')) {
                    loader.classList.add('opacity-0');
                    setTimeout(() => loader.remove(), 700);
        ...
        videoEl.onerror = () => {
            if (!isReady) {
                console.error(`❌ 视频加载失败: ${videoObj.src}`);
                isReady = true;
                const loader = document.getElementById('globalLoader');
                if (loader && !loader.classList.contains('opacity-0')) {
                    loader.classList.add('opacity-0');
    ...
    // ==========================================
    // 🖼️ 单图模式 (✨ 完美恢复 6.5 秒缓慢放大呼吸感)
    // ==========================================
    playClassicMode: function() {
        // ✨ 新增：播大图时，唤醒顶层玻璃球，形成全息悬浮感！
        ...
        // ✨ 呼吸感重构：初始大小为 1，准备在 6.5 秒内平滑放大到 1.05
        ...
        this.addSceneTimer(() => {
            ...
            imgEl.style.transform = 'scale(1.15)'; // ✨ 触发缓慢放大，呼吸感满血复活！
        }, 50);
    },
